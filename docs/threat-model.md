...
| Administrators | Approve/deny/renew/revoke decisions within their role (viewer, administrator, or application_owner) | Bypassing CSRF/role checks -- those are enforced regardless of who's asking |
...
- **Actors:** anonymous internet clients, browsers with a valid credential, administrators (viewer/administrator/application_owner roles), Traefik itself, application backends, the retention/cleanup worker (Milestone 5).
...
| Over-broadly-scoped `application_owner` account (an owner reading or mutating another application's requests/authorizations, or reaching staff-only endpoints like overview/audit-log) | Two layers: `requireStaffRole` blocks the whole role from endpoints with no per-resource check at all; every request/authorization-scoped read and mutation instead resolves the caller's owned-application set fresh from `application_owners` on every call (never cached on the session, so a revoked grant takes effect on the owner's very next request) and 404s -- not 403, to avoid confirming another application's resource IDs exist -- on a mismatch | `internal/httpserver.requireStaffRole/requireAdministratorOrOwner/authorizeOwnedResource`, `internal/store.GetOwnedApplicationIDs`, `migrations/000016_application_owners` | **Implemented and tested** (`internal/httpserver/admin_handlers_test.go`'s `TestApplicationOwner_*` cases; `internal/store/application_owners_test.go`) |
...
| A stolen/copied credential cookie used from a different device or network than the one it was issued to (the credential is a device's entire identity for up to a year -- see section 1's residual-risk note) | Revocation policy (not spec-mandated; a deliberate extension for the "unattended display, no device attestation" threat model this service targets): the IPChanged/UserAgentChanged signals are checked synchronously inside the same `Decide` call that would otherwise Allow, comparing the incoming request against the authorization's own last-seen values *before* they're overwritten -- a "revoke" outcome flips that same request's own answer to Deny, it is not a separate, delayed, or best-effort check. A small fixed enumerated action set (off/warn/flag_for_review/revoke), never a rule engine/DSL, keeps this logic auditable in a fail-closed hot path. Layered override (session -> application -> deployment default) lets a stationary digital sign and a roaming laptop have different defaults. | `internal/authz.Decide`/`resolveChangeSignal`, `internal/revokepolicy`, migration 000018 | **Implemented and tested** (`internal/authz/service_test.go`'s `TestDecide_IPChangedRevokesWhenConfigured` and the surrounding revocation-policy test group; `internal/store/revocation_policy_test.go`) |
| A device that stops checking in entirely (network loss, powered off) never triggering an in-path signal, so a `revoke`-configured session it should have caught never gets caught | A separate worker job (`EnforceInactivityPolicy`, same per-job advisory lock as every other retention job) scans for authorizations whose last activity exceeds a configured threshold and applies the same layered-override resolution `Decide` uses for the synchronous signals -- deliberately global-only for the threshold itself (not layered per-application/session) to bound this feature's scope; only the resulting action is layered | `internal/store.EnforceInactivityPolicy`; `cmd/server`'s `enforce_inactivity_policy` job | **Implemented and tested** (`internal/store/revocation_policy_test.go`'s `TestEnforceInactivityPolicy_*`) |
| A revocation-policy admin action (revoke/flag/warn) racing a concurrent admin action on the same authorization | The policy-triggered revoke reuses the authorization's own optimistic-concurrency version read in the same `GetAccessSnapshot` call; a version conflict resolves to Deny (the safe default when a concurrent change means the decision's own premise no longer holds), never propagated as a request-failing error | `internal/authz.Decide` | **Implemented and tested** (`TestDecide_RevokeConflictDeniesWithoutPropagatingAnError`) |
| Audit-log tampering via the retention/cleanup path itself | Audit-event purging (the one deletion the audit-immutability design permits at all) requires the separate `app_maintenance` role and runs only as a distinct one-shot operator command, never inside the always-running service process | `cmd/admin purge-audit-log`; migration 000012's grants | **Implemented and tested** (`internal/store/retention_test.go`'s `TestPurgeOldAuditEvents`, which explicitly connects as `approve_auth_maintenance`) |
| A stuck/unreachable notification destination (SMTP relay down, webhook endpoint hanging) degrading or blocking approval itself | Notifications are enqueued to a durable outbox in their own call, separate from the transaction that creates the request, and delivered by a background job with a bounded per-attempt timeout and capped exponential backoff -- never inline with, and never able to fail, the browser's own request submission or an admin's approve/deny/revoke call | `internal/notify.Dispatcher.DeliverPending`, `internal/enrollment.Service.enqueueRequestCreatedNotification` (best-effort, logs and swallows its own errors) | **Implemented and tested** (`internal/notify/job_test.go`, `internal/enrollment/service_test.go`'s `TestSubmitRequest_EnqueuesRequestCreatedNotification`) |
| An administrator pointing `notify_webhook_url` at an internal-only address (blind SSRF via this service's own outbound network position) | Not separately mitigated -- an administrator is already trusted with equivalent-or-greater capability (creating/disabling applications, granting ApplicationOwner, viewing the full audit log), and the webhook response body/headers are never reflected back to any admin-facing API (only a generic delivery success/failure is stored), so this adds no new trust boundary, only a blind-request primitive to an actor who could already reach that address by other means with this same level of access | -- | **By design, not a gap** -- see section 5 |
