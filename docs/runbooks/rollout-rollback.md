...
1. **Build and push the image.** `Dockerfile`'s default target
   ...
   ```bash
   docker build -t <registry>/manual-approval:<tag> .
   docker push <registry>/manual-approval:<tag>
   ```
   ...
   ```bash
   export MANUAL_APPROVAL_IMAGE="$(docker inspect --format='{{index .RepoDigests 0}}' <registry>/manual-approval:<tag>)"
   ```
...
2. **Label a storage node** for PostgreSQL (spec section 13: pin it to
   ...
   ```bash
   docker node update --label-add manual-approval-db-storage=true <node-id>
   ```
...
3. **Create secrets.** Every name below matches `deploy/stack.yml`'s
   ...
   | `database_url_superuser` | `postgres://<superuser>:<password>@db:5432/manual_approval?sslmode=require` -- used only by the one-shot `migrate` service |
   ...
   | `auth_tls_cert`, `auth_tls_key` | The Authorization listener's own server certificate/key (SAN must include the Swarm service name `manual-approval`) |
...
4. **Prepare the Traefik side.** Whoever manages the shared Traefik
   deployment mounts `deploy/traefik-manual-approval-middleware.yml`
   into its file-provider directory, with `manual_approval_auth_ca`/
...
5. **Write the real nonsecret config.** Copy
   `deploy/manual-approval-config.example.yaml`, fill in real values,
...
6. **Deploy:**
   ...
   docker stack deploy -c deploy/stack.yml manual-approval
   ```

   Watch `migrate` reach `Complete` before `manual-approval` starts
   ...
   ```bash
   docker stack ps manual-approval
   ```
...
7. **Register the first application** (spec section 13/14: "bootstrap
   ...
   docker run --rm --network manual-approval_db \
     ...
     -v <path-to-real-config>:/config/manual-approval.yaml:ro \
...
8. **Onboard that application's own Traefik routing** (spec section 6):
   add the reserved-path router and the `manual-approval@file`
...
```bash
export MANUAL_APPROVAL_IMAGE="<new digest>"
docker stack deploy -c deploy/stack.yml manual-approval
```
...
```bash
export MANUAL_APPROVAL_IMAGE="<previous known-good digest>"
docker stack deploy -c deploy/stack.yml manual-approval
```
